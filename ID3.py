...
def id3(dataset, attributes, target_attr, max_depth=None, min_samples_split=2, depth=0):
    """
    Build an ID3 decision tree recursively with majority class fallback.
    """
    # Calcula a classe mais comum neste nó para servir de fallback (segurança)
    majority_class = Counter(dataset[target_attr]).most_common(1)[0][0]
    ...
    # Se não houver ganho de informação, paramos aqui
    if information_gain(dataset, best_attr, target_attr) == 0:
        return majority_class

    # Criamos o nó com a chave especial _majority para o preditor usar depois
    tree = {best_attr: {"_majority": majority_class}}
    remaining = [a for a in attributes if a != best_attr]

    # Expansão dos ramos
    for val in dataset[best_attr].unique():
        sub = dataset[dataset[best_attr] == val]
        # Se o subset estiver vazio, este ramo leva à classe maioritária do pai
        if len(sub) == 0:
            tree[best_attr][val] = majority_class
        else:
...
def predict_sample(tree, sample, default="unknown"):
    """
    Predict the class of a single sample dict with fallback to node majority.
    """
    # Se chegámos a uma folha (string), retornamos o valor da jogada
    if not isinstance(tree, dict):
        return tree

    # root_attr será o nome da célula (ex: 'c21')
    root_attr = next(iter(tree))
    
    attr_value = str(sample.get(root_attr, ""))

    # subtree_dict contém os ramos (0, 1, 2) e a chave '_majority'
    subtree_dict = tree[root_attr]

    # Se o valor da célula (ex: '2') não foi visto no treino deste nó
    if attr_value not in subtree_dict:
        # Retornamos a maioria guardada no nó, ou o default caso algo falhe
        return subtree_dict.get("_majority", default)

    # Se existe o ramo, continuamos a descer na árvore
    return predict_sample(subtree_dict[attr_value], sample, default)
...
def count_nodes(tree):
    ...
    leaves = 0
    internal = 1
    for key, sub in tree[attr].items():
        if key == '_majority':  # Skip fallback metadata — not a real structural node
            continue
        l, i = count_nodes(sub)
        leaves += l
        internal += i
...
def get_informative_features(dataset, target='move', min_gain=0.001):
    """Devolve só as features com information gain acima do mínimo."""
    feats = [f'c{i}' for i in range(42)]
    useful = [f for f in feats if information_gain(dataset, f, target) > min_gain]
    print(f"Features úteis: {len(useful)}/42")
    return useful
...
def train_popout_tree(dataset, max_depth=12, min_samples_split=5):
    """Train ID3 on PopOut board data. Target: move ("drop_3", "pop_2", etc.)"""
...
def evaluate_tree(tree, test_data, target_col='move'):
...
if __name__ == "__main__":
    ...
    if df is None:
        ...
        drop_moves = sum(count for move, count in mv.items() if 'drop' in str(move))
        pop_moves = sum(count for move, count in mv.items() if 'pop' in str(move))
        print(f"Drop Moves: {drop_moves}  Pop Moves: {pop_moves}")

        train_p, test_p = train_test_split_manual(df, test_size=0.2)
        feats_p = get_informative_features(train_p)
        tree_p = train_popout_tree(train_p, max_depth=15, min_samples_split=5)
        ...
        df_copy = df.copy()
        df_copy['move'] = df_copy['move'].astype(str)
        cv_p = kfold_cross_validation(df_copy, feats_p, 'move', k=5, max_depth=20)
